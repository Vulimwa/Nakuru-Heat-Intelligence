...
const intentKeywords: Array<{ intent: string; keywords: string[] }> = [
  ...
  },
  {
    ...
    keywords: ["population", "exposed", "people", "exposure"],
  },
  {
    intent: "land_cover",
    keywords: [
  ...
  {
    intent: "methodology",
    keywords: ["methodology", "analysis", "dataset", "approach", "process"],
  },
  {
  ...
  {
    intent: "general_observatory_question",
    keywords: ["what", "when", "how", "why", "overview", "summary", "findings"],
  },
];
...
function buildKnowledgeDocument(markdown: string): KnowledgeDocument {
  ...
  const flushSection = () => {
    ...
    const path = [...documentStack.map((item) => item.title), currentTitle]
      .filter(Boolean)
      .join(" > ");
    ...
    section.keywords = Array.from(
      new Set(
        tokenize(`${section.title} ${section.content}`).filter(
  ...
  for (const line of lines) {
    ...
    if (headerMatch) {
      ...
      while (
        ...
        documentStack[documentStack.length - 1].level >= currentLevel
      ) {
        documentStack.pop();
      }
  ...
  return (
    document.sections.find((section) =>
      normalizedTitles.some(
        (title) =>
          normalize(section.title).includes(title) ||
          normalize(section.path).includes(title),
      ),
    ) || null
  );
}
...
function disambiguateQuestion(question: string): {
  entities: string[];
  ambiguous: boolean;
} {
  const normalized = normalize(question);
  ...
  if (
    normalized.includes("area of nakuru") &&
    !normalized.includes("county") &&
    !normalized.includes("city") &&
    !normalized.includes("lake") &&
    !normalized.includes("aoi")
  ) {
    ambiguous = true;
  }
...
function getHeatAreaTable(document: KnowledgeDocument): KnowledgeTable | null {
  return firstTable(
    ...
    ["heat area", "heat area changes", "heat-area changes"],
  );
}

function getPopulationExposureTable(
  document: KnowledgeDocument,
): KnowledgeTable | null {
  return firstTable(
    document,
    ["low", "moderate", "high", "very high"],
    ["population exposure", "population exposed"],
  );
}
...
function getLandCoverTable(document: KnowledgeDocument): KnowledgeTable | null {
  return firstTable(
    document,
    ["mean siuhi 2021", "mean siuhi 2026"],
    ["siuhi by land cover", "land cover"],
  );
}

function getSiuhiStatsTable(
  document: KnowledgeDocument,
): KnowledgeTable | null {
  return firstTable(
    document,
    ["minimum siuhi", "maximum siuhi", "mean siuhi", "standard deviation"],
    ["siuhi statistics", "siuhi statistics"],
  );
}

function getMeanSiuhiValue(
  document: KnowledgeDocument,
  year: string,
): number | null {
  const table = getSiuhiStatsTable(document);
  if (!table) return null;
  const yearIndex = table.headers.findIndex((header) =>
    normalize(header).includes("year"),
  );
  const meanIndex = table.headers.findIndex((header) =>
    normalize(header).includes("mean siuhi"),
  );
  if (yearIndex < 0 || meanIndex < 0) return null;
  const row = table.rows.find((row) =>
    normalize(row[yearIndex]).includes(normalize(year)),
  );
  return row ? extractNumber(row[meanIndex]) : null;
}

function getLakeDistanceTable(
  document: KnowledgeDocument,
): KnowledgeTable | null {
  return firstTable(
    document,
    ["distance band", "siuhi 2026"],
    ["lake nakuru", "lake area"],
  );
}
...
function getTableValue(
  ...
  rowMatch: (row: string[]) => boolean,
  columnMatch: (header: string) => boolean,
): string | null {
...
function getHeatAreaValue(
  ...
  const classIndex = table.headers.findIndex((header) =>
    normalize(header).includes(normalize(heatClass)),
  );
  if (yearIndex < 0 || classIndex < 0) return null;
  const row = table.rows.find(
    (row) => normalize(row[yearIndex]) === normalize(year),
  );
  return row ? extractNumber(row[classIndex]) : null;
}

function getPopulationExposureValue(
  document: KnowledgeDocument,
  year: string,
  heatClass: string,
): number | null {
...
function answerLargestVeryHighYear(document: KnowledgeDocument): string {
  ...
  const field = Object.keys(series[0]?.values || {}).find((key) =>
    normalize(key).includes("very high"),
  );
  if (!field) return "I couldn't identify the Very High heat column.";
  const best = series.reduce((prev, current) =>
    current.values[field] > prev.values[field] ? current : prev,
  );
  return `The largest Very High heat area occurred in ${best.year}, at approximately ${formatNumber(best.values[field])} km².`;
}

function answerPopulationExposure(
  document: KnowledgeDocument,
  year: string,
  heatClass: string,
): string {
...
function answerLandCoverHottest(
  document: KnowledgeDocument,
  year: string,
): string {
  ...
  const yearHeader = table.headers.find((header) =>
    normalize(header).includes(normalize(year)),
  );
  if (!yearHeader) {
...
function answerLandCoverCoolest(
  document: KnowledgeDocument,
  year: string,
): string {
  const table = getLandCoverTable(document);
...
function answerLandCoverTrend(document: KnowledgeDocument): string {
  ...
  const changeHeader = table.headers.find((header) =>
    normalize(header).includes("change"),
  );
  if (!changeHeader) {
...
function answerCoolingEffect(document: KnowledgeDocument): string {
  ...
  const bandIndex = table.headers.findIndex((header) =>
    normalize(header).includes("distance"),
  );
  const siuhiIndex = table.headers.findIndex((header) =>
    normalize(header).includes("siuhi 2026"),
  );
...
function answerGeographicDistinction(document: KnowledgeDocument): string {
  const section = findSection(document, [
    "geographic context",
    "how to answer geographic questions",
...
function answerVeryHighTrend(document: KnowledgeDocument): string {
  ...
  if (!field) return "I couldn't identify the Very High heat column.";
  const ordered = series.filter((entry) =>
    Number.isFinite(entry.values[field]),
  );
...
export function answerLocalQuestion(question: string): string {
  ...
  if (
    normalized.includes("area of interest") ||
    normalized.includes("aoi") ||
    normalized.includes("observatory study area") ||
    intent === "study_area_information"
  ) {
  ...
  if (
    normalized.includes("lake") &&
    (normalized.includes("cooling") || normalized.includes("distance"))
  ) {
  ...
  if (
    ...
    return formatManagedAnswer(
      answerHeatAreaYear(document, "2026", "Very High"),
    );
  }
  ...
  if (
    intent === "heat_area" &&
    normalized.includes("very high") &&
    normalized.includes("which year")
  ) {
  ...
  if (
    ...
    return formatManagedAnswer(
      answerHeatAreaYear(document, "2025", "Very High"),
    );
  }
  ...
  if (
    ...
    return formatManagedAnswer(
      answerPopulationExposure(document, "2026", "Very High"),
    );
  }
  ...
  if (
    normalized.includes("cooling effect") ||
    normalized.includes("lake cooling")
  ) {
    return formatManagedAnswer(answerCoolingEffect(document));
  }
  ...
  if (
    normalized.includes("difference between nakuru county") ||
    normalized.includes("difference between nakuru city") ||
    normalized.includes("difference between nakuru")
  ) {
    return formatManagedAnswer(answerGeographicDistinction(document));
  }

  if (
    (normalized.includes("increase or decrease") ||
  ...
  if (
    ...
    normalized.includes("very high") &&
    normalized.includes("not necessarily")
  ) {
    return formatManagedAnswer(answerRiskInterpretation());
  }

  if (
    normalized.includes("planning interventions") ||
  ...
  if (
    ...
    normalized.includes("study area")
  ) {
  ...
  if (disambiguation.ambiguous) {
    return formatManagedAnswer(
      "The request is ambiguous. The knowledge base distinguishes Nakuru County, Nakuru City, the Observatory Area of Interest, and Lake Nakuru. Please clarify which entity you mean.",
    );
  }
